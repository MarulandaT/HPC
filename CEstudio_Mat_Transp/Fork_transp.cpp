...
int main (int argc, char *argv[]){
    ...
    D = (int **) mmap(NULL,sizeof(int *)*number,PROT_READ | PROT_WRITE,MAP_SHARED | MAP_ANONYMOUS,1,0);
    ...
    for (int i = 0; i< number;i++){
        ...
        D[i] = (int*)mmap(NULL,sizeof(int *)*number,PROT_READ | PROT_WRITE,MAP_SHARED | MAP_ANONYMOUS,1,0);
    }

    create(A,number);
    create(B,number);
    transponer(D, B, number);
